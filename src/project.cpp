...
void fcfs(std::vector<Process> &processes); // first come first serve
...
int main(int argc, char** argv){
    ...
    * */
    ...
    {
        int bCount =  20;//next_uni(1, 100); //number of CPU bursts, a random integer between 1 and 100
        int arrTime = 30-p;//next_uni(1, 200);; //arrival time in milliseconds.  UNIMPLEMENTED.
        ...
        for (int i = 0; i < bCount; i++){
            ...
            processes.back().addTimes(cpuTime+i, ioTime);
    ...
    int startsreached =0;
        while(inprocess){
           ...
           if (inuse == false){
            ...
            if(ioState.size()==0 && readyState.size()==0 && eventlog.size()==0){
                inprocess =false;
                continue;
            }
            if(currtime > 650){
            ...
            std::cout<< "time is ->" << currtime<< std::endl;
            currtime+=1;
    }
    /*
    Basic Algorithm:
